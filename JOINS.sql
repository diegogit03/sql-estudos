...
-- Left Outer Join
-- Mostraria um cliente mesmo se não tivesse profissão
SELECT * FROM clientes
LEFT OUTER JOIN profissoes
ON clientes.id_profissao = profissoes.id;

-- Right Outer Join
-- Mostra todos os cargos, mesmo se não tiverem clientes associados
SELECT * FROM clientes
...
-- Cross Join
-- Ele simplesmente junta todos os dados sem filtros
SELECT c.id, c.nome, c.data_nascimento, c.telefone, p.cargo
FROM clientes AS c
...
SELECT a.nome AS Consumidor1, b.nome AS Consumidor2, a.cidade
	FROM consumidor AS a INNER JOIN consumidor AS b
	ON a.id <> b.id
